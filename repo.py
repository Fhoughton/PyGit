...
"""
    ...
    def read_object(self, hash):
        ...
        object_path = self.gitdir_path("objects", sha[0:2], sha[2:]) # sha [0:2] as first 2 characters of hash are folder name, then rest file name

        with open(object_path, "rb") as object_bin:
            object_data = zlib.decompress(object_bin.read()) # By default git objects are compressed
            ...
            null_index = object_data.find(b'\x00', space_index) # Start searching after the space, technically not needed
            ...
            match object_type:
                ...
                case b'tree': pass
                case b'tag': pass
                case _: raise Exception(f"Unrecognized object type for {hash}, found '{object_type}' not commit, blob, tree or tag")
    ...
    def write_object(self, obj):
        ...
        object_data = obj.get_name() + b' ' + str(len(object_data)).encode() + b'\x00' + object_data

        # Hash the object
        object_hash = hashlib.sha1(object_data).hexdigest()

        # Resolve the path for the object to be stored in
        object_path = self.gitdir_path("objects", object_hash[0:2], object_hash[2:], mkdir=True)
